...
export const getAnimeDetails = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: storedLinks } = await supabaseAdmin
      .from("media_links")
    ...
    const storedEpisodes = Array.from(new Set((storedLinks ?? []).map((l) => l.episode_number)));
    const availableEpisodes = storedEpisodes.length > 0 ? storedEpisodes : Array.from({ length: total }, (_, i) => i + 1);
